    ...
    def step(self, closure: Optional[Callable] = None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                if p.grad is None:
                    continue
                state = self.state[p] # p自身就是参数(tensor)，state保存的是和p相关的动态参数，比如训练step数等
                # ========== 训练刚开始初始化state ========== #
                if len(state) == 0:
                ...
                state["t"] += 1
                t = state["t"] # dtype: int
                m, v = state["m"], state["v"] # dtype: torch.Tensor
                grad = p.grad # dtype: torch.Tensor
                # m = beta1 * m + (1.0 - beta1) * grad
                # v = beta2 * v + (1.0 - beta2) * torch.pow(grad, 2)
                # 使用in-place方式更新m, v
                m.mul_(beta1).add_((1 - beta1) * grad)
                v.mul_(beta2).add_((1.0 - beta2) * torch.pow(grad, 2))
                term1 = math.sqrt(1.0 - math.pow(beta2, t))
                term2 = 1.0 - math.pow(beta1, t)
                lr_t = lr * term1 / term2
                # p.data -= lr_t * m / (torch.sqrt(v) + eps)
                # p.data -= lr * weight_decay * p.data
                # 使用in-place方式更新p；注意p使用in-place修改时，如果使用p.addcdiv_()会报错，需要使用p.data.addcdiv_()
